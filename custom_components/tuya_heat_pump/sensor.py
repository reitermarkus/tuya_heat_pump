...
class TuyaHeatpumpSensor(SensorEntity):
    ...
    def native_value(self) -> str | float | None:
        ...
        if self._config["code"] == "calculated_power":
            return self._calculate_power()

        if (
            not self.coordinator.data
            or self._config["code"] not in self.coordinator.data
        ):
            return None

        raw_value = self.coordinator.data[self._config["code"]]["value"]
    ...
    def available(self) -> bool:
        ...
        if self._config["code"] in ["calculated_power", "total_energy"]:
